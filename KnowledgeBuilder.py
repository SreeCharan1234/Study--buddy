...
roadmap=Agent(name="roadmap")
ai_bot=Agent(name="ai_bot")
code_editor=Agent(name="code_editor")
mock_interview=Agent(name="mock_interview")



@roadmap.on_event("startup")
async def introduce_alice(ctx: Context):
    ctx.logger.info(f"Hello, I'm agent {roadmap.name}.")
    example2()
...
@ai_bot.on_event("startup")
async def introduce_bob(ctx: Context):
    ctx.logger.info(f"Hello, I'm agent {ai_bot.name}.")
...
@code_editor.on_event("startup")
async def introduce_alice(ctx: Context):
    ctx.logger.info(f"Hello, I'm agent {code_editor.name}.")
...
@mock_interview.on_event("startup")
async def introduce_bob(ctx: Context):
    ctx.logger.info(f"Hello, I'm agent {mock_interview.name}.")
    main()
...
if selected=="Code Editor": 
    bureau.add(code_editor)
if selected== "Mock Interview":
    bureau.add(mock_interview)
